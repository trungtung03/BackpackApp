...
import com.example.backpackapp.view.location.RequestsAdapter.ViewHolder
import kotlinx.android.synthetic.main.item_rcv_requests_trip_location.view.*
import java.text.SimpleDateFormat
import java.util.*
import kotlin.collections.ArrayList

class RequestsAdapter(private val mContext: Context) :
        ...
        override fun bindViewHolder(context: Context?, data: LocationTripModel?, position: Int) {
            if (context != null) {
                ...
                with(context).load(data?.vehicle)
                    .into(itemView.item_rcv_requests_trip_item_airplane_mode)
            }

            data?.countOverview.toString().also {
                itemView.item_rcv_requests_trip_tv_count.text = it
            }
            Date.parse(data?.dateTime).let { SimpleDateFormat(PATTERN_DATE_FORMAT).format(it) }
                .also {
                    itemView.item_rcv_requests_trip_tv_date_time.text =
                        it
                }
            itemView.item_rcv_requests_trip_text_from_country.setCountryForNameCode(
                data?.fromCountry
            )
            itemView.item_rcv_requests_trip_tv_to_country.setCountryForNameCode(
                data?.toCountry
            )
        }
    }
